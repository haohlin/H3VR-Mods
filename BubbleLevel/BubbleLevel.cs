...
namespace HLin_Mods.BubbleLevelSet
{
    public class BubbleLevel : MonoBehaviour {
        ...
        private void Update()
        {
            ...
            // }
            float cur_z_angle;
			if (this.attachment != null && this.attachment.curMount != null)
			{
				cur_z_angle = this.attachment.curMount.GetRootMount().transform.eulerAngles.z;
			...
			{
				cur_z_angle = this.baseObject.transform.eulerAngles.z;
				if (180f < cur_z_angle && cur_z_angle < 360f)
				{
					cur_z_angle -= 360f;
				}
			}
			Vector3 localPosition = this.level_bubble.transform.localPosition;
			localPosition.x = Mathf.Clamp(-0.0084f + cur_z_angle / 8f, -0.33f, 0.33f);
			this.level_bubble.transform.localPosition = localPosition;            
        }
    }
}
