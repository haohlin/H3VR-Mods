...
| Completion-aware retry candidate | Live source proves `CurrentlySpawnedRandomGun` is set before vanilla `SpawnRandomGunRoutine` creates feed/attachments. The prior code could accept a non-firearm or race its feed scan; it had no retry. | Current candidate wraps the native routine, validates registered `FVRFireArm` plus an actually loaded feed, retries only completed failures up to three times, preserves native gear on timeout/final failure, and never bulk-deletes object-snapshot deltas. Windows verification pending. |
...
| `[ ]` | Build, verify, deploy completion-aware random retry. | Native random routine completion is observed; invalid completed gun/feed results reroll the same tier at most three times; timeout/final failure preserves native equipment; only Cursed-owned objects are cleaned. |
...
| Completion-aware retry candidate | `h3vr-remote run Test`; `Verify GunGameCursedRandom`; `Build GunGameCursedRandom`; `Deploy GunGameCursedRandom` | Pending. Source regression requires native-routine completion tracking, three-attempt cap, actual firearm/feed validation, no `Promote()` skip, and native-equipment preservation on timeout/final failure. |
