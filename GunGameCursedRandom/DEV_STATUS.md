...
| Overlapping handoff regression | Fresh VR trace: normal guns work, then `Sustenance AR3` appears as random result and cleanup completes but no hand-equip/loadout log follows before next random request. `spawningRandomGun` was cleared before cleanup/feed work, allowing event overlap and NullReference flood. | Root cause confirmed; source candidate keeps full transition pending, equips firearm before optional feeds, catches feed failure, and queues later progression; Windows checks/deploy pending. |
...
| `[ ]` | Serialize random handoff and deploy. | Every random result either reaches selected hand or logs explicit disappearance; later promotions cannot overlap handoff; feed failure keeps firearm. |
