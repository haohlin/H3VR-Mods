...
| `[ ]` | Restore guarded native cleanup and reliable feed ordering. | Native G17 gun/magazine absent; M240 loads `Magazine M240` and gets spare; immediate random result never receives `DestroyOldEq`; static regression checks enforce order. |
...
| Native-cleanup / feed-order repair | `h3vr-remote run Test`; `Verify GunGameCursedRandom`; `Build GunGameCursedRandom`; `Deploy GunGameCursedRandom` | Pending. Required regression checks: API invocation precedes guarded `DestroyOldEq`, random-result wait follows it, loose wrapper-backed feed precedes attached fallback. |
