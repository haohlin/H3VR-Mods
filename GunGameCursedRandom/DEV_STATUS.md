...
| Completion-aware retry candidate | Live source proves `CurrentlySpawnedRandomGun` is set before vanilla `SpawnRandomGunRoutine` creates feed/attachments. The prior code could accept a non-firearm or race its feed scan; it had no retry. | Current candidate wraps native completion, accepts only registered firearm plus a native generated/loaded feed, retries completed failures up to three times, destroys only failed attempt identities, restores native equipment after direct suppression failure, and blocks new Cursed requests until a timed-out routine exits. Windows verification pending. |
...
| `[ ]` | Build, verify, deploy completion-aware random retry. | Native random routine completion is observed; invalid completed gun/feed results reroll the same tier at most three times; direct timeout/final failure restores native equipment; only exact failed-attempt result/feed identities are cleaned. |
| `[ ]` | Deploy guarded native cleanup and feed ordering. | Deploy guard confirms H3VR stopped; Default receives latest DLL; logs clear before VR test. |
...
| Completion-aware retry candidate | `h3vr-remote run Test`; `Verify GunGameCursedRandom`; `Build GunGameCursedRandom`; `Deploy GunGameCursedRandom` | Pending. Source regression requires native-routine completion tracking, three-attempt cap, native firearm/feed validation, no `Promote()` skip, timeout inflight gate, and native-equipment preservation on direct timeout/final failure. |
...
- [ ] Latest native-cleanup / feed-order DLL deployed after H3VR-stop guard.
