...
Native GunGame WeaponBuffer.SpawnAsync
  ...
  -> validate a real registered FVRFireArm and feed generated by that native
     routine that actually loads
...
- Use the live `ItemSpawnerV2` random-gun API; do not maintain a second random
  weapon, attachment, or ammo selector.
...
- Failed attempts destroy only their result root and exact loose feeds captured
  from that same native routine; accepted native loose feeds remain in place.
...
- A direct `SpawnAsync` interception restores native GunGame equipment after a
  startup failure, timeout, or exhausted retry budget. A timed-out native
  routine blocks new Cursed random requests until its wrapped coroutine exits.
...
| Validate native feed, never synthesize one. | A second `GetRandomAmmoObject` call can mask a broken native result and compile against a different game API. Failed native feed generation rerolls the same tier instead. | 2026-07-22 |
| Track failed attempt identities. | Result root plus exact post-attempt loose feeds permit cleanup without broad scene scans or player quickbelt deletion. | 2026-07-22 |
