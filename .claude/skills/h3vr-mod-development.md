...
The repository command is `powershell.exe -ExecutionPolicy Bypass -File E:\Dev\H3VR-Mods\tools\h3vr.ps1`. Its normal release sequence is `Test`, `Package`, `Deploy`, manual VR test, `TailLogs`, then the separately gated `Publish` action.
